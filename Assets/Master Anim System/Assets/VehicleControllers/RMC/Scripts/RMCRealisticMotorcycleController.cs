...
public class RMCRealisticMotorcycleController : MonoBehaviour {
	...
	public void SoundsInitialize (){
		...
		engineAudio = CreateAudioSource("engineSound", 10, 0, engineClip, true, true, false);
		skidAudio = CreateAudioSource("skidSound", 5, 0, skidClip, true, true, false);
		//engineStartAudio = CreateAudioSource("engineStartSound", 5, .2f, engineStartClip, false, true, true);
	...
	void Engine (){
		...
		//Engine Audio Volume.
		engineAudio.volume = Mathf.Lerp (engineAudio.volume, (Mathf.Clamp (motorInput, .35f, .85f))*AudioVolume, Time.deltaTime*  5);
		engineAudio.pitch = Mathf.Lerp ( engineAudio.pitch, (Mathf.Lerp (1f, 2f, (EngineRPM - (MinEngineRPM / 1.5f)) / (MaxEngineRPM + MinEngineRPM))) * AudioVolume, Time.deltaTime * 5);
		...
		if (collision.contacts.Length > 0){
			...
			if(collision.relativeVelocity.magnitude > 10 && crashClips.Length > 0){
				...
				if (collision.contacts[0].thisCollider.gameObject.transform != transform.parent){
					...
					crashAudio = CreateAudioSource("crashSound", 5, 0.3f, crashClips[UnityEngine.Random.Range(0, crashClips.Length)], false, true, true);
